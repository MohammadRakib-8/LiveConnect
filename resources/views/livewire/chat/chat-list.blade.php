<div
   ...
   class="flex flex-col h-full bg-white overflow-hidden">
    ...
    <!-- Header -->
    <header class="px-5 z-10 bg-white/90 backdrop-blur-md sticky top-0 w-full py-4 border-b border-gray-100">
        ...
        <div class="flex gap-3 items-center">
            <button @click="type='all'" :class="{'bg-black text-white shadow-lg transform scale-105':type=='all'}" class="transition-all duration-300 ease-out inline-flex justify-center items-center rounded-full px-5 py-2 text-sm font-medium border border-transparent bg-gray-100 hover:bg-gray-200 text-gray-600">
                All
            </button>
            <button @click="type='deleted'" :class="{'bg-black text-white shadow-lg transform scale-105':type=='deleted'}" class="transition-all duration-300 ease-out inline-flex justify-center items-center rounded-full px-5 py-2 text-sm font-medium border border-transparent bg-gray-100 hover:bg-gray-200 text-gray-600">
                Deleted
            </button>
        </div>
    </header>
    ...
    <!-- Main List -->
    <main class="overflow-y-scroll flex-1 p-10 space-y-1 bg-white-100">
        @if ($conversations)
            @foreach ($conversations as $key => $conversation)
                <li
                    wire:key="{{$conversation->id}}"
                    wire:click="selectConversation({{$conversation->id}})"
                    class="group flex items-center gap-4 p-1 rounded-2xl cursor-pointer transition-all duration-200 hover:bg-gray-50 hover:shadow-sm {{ $conversation->id == $selectedConversation?->id ? 'bg-blue-50/80 shadow-sm ring-1 ring-blue-100' : '' }}"
                >
